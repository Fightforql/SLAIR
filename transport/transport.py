...
class Transport:
    ...
    def sample(self, x1, sp_timesteps=None, shifted_mu=0, timestep_shift=1.0):
        ...
        # Apply timestep shift: same logic as inference (integrators.py)
    ...
    def training_losses(
        ...
        t, x0, x1 = self.sample(x1, sp_timesteps, shifted_mu, timestep_shift) # x0 = noise, x1 = clear
        t, xt, ut = self.path_sampler.plan(t, model_kwargs['y'], x1) #

        # Add BBDM noise: xt = xt + sqrt(t * (1 - t)) * x0
        bridge_std = th.sqrt(t * (1 - t))
        noise = th.randn_like(xt) * path.expand_t_like_x(bridge_std, xt)
        xt = xt + noise

        model_output = model(xt, t, y=model_kwargs['inputs'], label=model_kwargs['label'])
        # print(model_output.shape, xt.shape, t.shape)
        # ut_ = (model_output - xt) / th.clamp(1 - expand_t_like_x(t, xt), 0.05)
        # model_output = ut_
        B, *_, C = xt.shape
        ...
        # y_gt = th.sqrt(th.mean((x0 - x1) ** 2, dim=(1, 2, 3))) / 2
        # print(y_gt, ty)
        # terms['y_loss'] = th.mean((y_gt - ty) ** 2)
        return terms
    ...
    def get_drift(
        ...
        def body_fn(x, t, model, **model_kwargs):
            ...
            assert model_output.shape == x.shape, f"Output shape from ODE solver must match input shape. Got output shape {model_output.shape}, expected {x.shape}"
            return model_output
...
class Sampler:
    ...
    def sample_ode(
        ...
        *,
        ...
        - flow_threshold: threshold for early stopping. If relative update (||flow||/||x||) < threshold, return early without denoising
        """
        if reverse:
